...
<body>
<form action="${pageContext.request.contextPath}/emp/list/1"
		id="empform">
</form>
		...
		<table class="layui-table" id="tbdata" lay-filter="tbop">
			...
			<c:forEach items="${pageBean.list }" var="emp">
				...
				<tr>
					...
					<td><a class="layui-btn layui-btn-mini" href="${pageContext.request.contextPath }/emp/goUpdate/${emp.no}">编辑</a>
						<a class="layui-btn  layui-btn-danger" onclick="deleteCourse(${emp.no});" >删除</a>
					</td>
				</tr>
			</c:forEach>
			</tbody>
...
<script type="text/javascript">
	...
	function goPage(pageIndex){
		...
		$("#empform").attr("action","${pageContext.request.contextPath}/emp/list/"+pageIndex+"?pageSize="+pageSize);
	...
	function changeSize(obj){
		var size=obj.value;
		$("#empform").attr("action","${pageContext.request.contextPath}/emp/list/1?pageSize="+size);
		$("#empform").submit(); //提交表单参数
	}
	...
	function deleteCourse(delId){
		//alert(delId);
		 layui.use('table', function() {
			   //alert(delId);
			   layer.confirm('是否确认删除该员工?',function(index) {
				   location.href="${pageContext.request.contextPath}/emp/del/"+delId;
			   });
			})
